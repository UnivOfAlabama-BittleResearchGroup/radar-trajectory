    ...
    def correct_center(self, df: pl.DataFrame) -> pl.DataFrame:
        return (
            df.with_columns(
                [
                    (
                        pl.col("f32_directionY")
                        / (
                            pl.col("f32_directionX").pow(2)
                            + pl.col("f32_directionY").pow(2)
                        ).sqrt()
                    ).alias("alpha"),
                ]
            )
            .with_columns(
                [
                    (
                        ...
                        - (
                            pl.col("f32_distanceToFront_m") * pl.col("alpha").sin()
                            + (pl.col("f32_length_m") / 2 * pl.col("alpha").sin())
                        )
                    ).alias("f32_positionX_m"),
                    (
                        pl.col("f32_positionY_m")
                        - (
                            pl.col("f32_distanceToFront_m") * pl.col("alpha").cos()
                            + (pl.col("f32_length_m") / 2 * pl.col("alpha").cos())
                        )
...
class Fusion:
    ...
    def _merge_radar(self, df: pl.DataFrame, target_pair: RadarHandoff) -> pl.DataFrame:
        ...
        #     pl.col("h3")
        # ])
        diff_columns = ["utm_x", "utm_y", "f32_velocityInDir_mps"]
        ...
        return (
            ...
            #     [
            #         *(
            #             pl.col(f"utm_{dim}" + _ext)
            #             .diff()
            #             .backward_fill()
            #             .over(["object_id_search", "object_id"])
            #             .alias(f"diff_{dim}{_ext}")
            #             for dim in ["x", "y"]
            #             for _ext in ["", "_search"]
            #         ),
            #     ]
            ...
            #                 (
            #                     (
            #                         pl.col("diff_y" + _ext) / pl.col("diff_x" + _ext)
            #                     ).arctan()
            #                 )
            #             ).alias("angle" + _ext)
            #             for _ext in ["", "_search"]
            ...
            .with_columns(
                [
                    *(
                    ...
                    #     (
                    #         (
                    #             pl.when(pl.col(f"diff_x{_ext}") > 0)
                    #             .then(pl.col(f"angle{_ext}"))
                    #             .when(
                    #                 (pl.col(f"diff_x{_ext}") < 0)
                    #                 & (pl.col(f"diff_y{_ext}") >= 0)
                    #             )
                    #             .then(pl.col(f"angle{_ext}") + math.pi)
                    #             .when(
                    #                 (pl.col(f"diff_x{_ext}") < 0)
                    #                 & (pl.col(f"diff_y{_ext}") < 0)
                    #             )
                    ...
                    #                 & (pl.col(f"diff_y{_ext}") != 0)
                    #             )
                    #             .then(pl.lit(math.pi) * pl.col(f"diff_y{_ext}").sign())
                    #             .otherwise(None)
                    #             .fill_null(0)
                    #             + math.pi
                    #         )
